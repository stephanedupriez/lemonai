...
function updateAction(message, messages) {
    ...
    if (action_index !== -1) {
        ...
        if (message.meta.action_type === 'read_url') {
          // On remplace le placeholder "Read URL ..." par le vrai contenu renvoyé par le runtime
          // (string ou array selon ton format)
          actions[action_index].content = message.content;
        }
    } else {
