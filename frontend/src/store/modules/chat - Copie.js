...
export const useChatStore = defineStore('chat', {
  state: () => ({
    ...
    mode_editor: false,
    /**
     * Modèle rapide (historique)
     */
    model_id: '',
    /**
     * Modèle de raisonnement (nouveau).
     * UI-only dans un premier temps : on le stocke ici pour que l’UI puisse le binder.
     * Tant que le backend n’est pas branché, on ne l’envoie nulle part.
     */
    reasoning_model_id: '',
    chatInfo: {
      pid: -1,
  ...
  actions: {
    async init(mode = 'task') {
      ...
      // Default: si aucun modèle de raisonnement n’est défini, on le cale sur le modèle rapide
      if (!this.reasoning_model_id && this.model_id) {
        this.reasoning_model_id = this.model_id;
      }
    ...
    if (get_res && get_res.title !== undefined) {
    ...
    async createConversation(message, mode_type = 'task') {
      ...
      const result = await chat.create(message, mode_type, this.agent.id, this.model_id, this.reasoning_model_id);
      this.chat = result;
      this.conversationId = result.conversation_id;

// Ne pas bloquer le flux UI : best-effort en arrière-plan
this.init().catch(e => console.warn("[chatStore.init] failed:", e));
this.autoTitle().catch(e => console.warn("[chatStore.autoTitle] failed:", e));
