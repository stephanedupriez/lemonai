...
/**
 * 🔍 DEBUG : log action + thinking
 */
onMounted(() => {
  console.log("[Observation] action =", props.action);
  console.log("[Observation] meta.thinking =", props.action?.meta?.thinking);
});
...
const actionTypeDescriptions = {
  ...
  patch_code: t("lemon.message.editFile"),
  replace_code_block: t("lemon.message.editFile"),
  browser: t("lemon.message.browsing"),
  web_search: t("lemon.message.searching"),
  read_url: t("lemon.message.browsing"),
...
const iconComponent = computed(() => {
  const actionType = props.action.meta.action_type;
  return svgHash[actionType] || Tools;
});
