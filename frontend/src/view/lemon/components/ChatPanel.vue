...
const handleSendMessage = async (value) => {
  ...
  try {
    ...
    const convId = chatStore.chat?.conversation_id;

    if (!convId) {
      throw new Error("No conversation_id in chatStore.chat");
    }

    if (preview) {
      editorStore.setShowSelectionPreview(false);
      await withTimeout(
        sse_coding.sendMessage(text, convId, files, mcp_server_ids, workMode),
        60000,
        "sse_coding.sendMessage"
      );
      return;
    }

    await withTimeout(
      seeAgent.sendMessage(text, convId, files, mcp_server_ids, workMode),
      60000,
      "seeAgent.sendMessage"
    );
  } catch (err) {
    console.error("[ChatPanel] handleSendMessage failed:", err);
    // Ici, si tu as un système de toast/notification dans ton projet,
    // c’est l’endroit idéal pour afficher un message utilisateur.
  }
};
