...
// - If used as <ModelSelect v-model:modelValue="..." />, the component will read/write the external modelValue.
// - If used as <ModelSelect /> (legacy behavior), it will read/write chatStore.model_id.
...
const { model_id, reasoning_model_id, conversationId } = storeToRefs(chatStore)

// IMPORTANT:
// Ne pas tenter de détecter v-model via useAttrs()/onUpdate:modelValue :
// dans Vue 3, les events déclarés via defineEmits ne remontent pas de manière fiable dans attrs.
// Règle fiable: si le parent passe modelValue (donc !== undefined), alors on est en mode v-model externe.
const hasExternalModel = computed(() => props.modelValue !== undefined)

// Single source of truth for the selected model ID (external v-model OR chatStore.model_id)
const boundModelId = computed({
  get() {
    return hasExternalModel.value ? props.modelValue : model_id.value
  },
  set(v) {
    if (hasExternalModel.value) emit('update:modelValue', v)
    else model_id.value = v
  },
})
...
const initModel = async () => {
  ...
  if (cachedData) {
    try {
      ...
      // If no model has been selected yet, default to the first model
      if (modelList.value.length > 0 && !boundModelId.value) {
        const defaultId = modelList.value[0].id * 1
        //model_id.value = defaultId
        boundModelId.value = defaultId
  ...
  try {
    ...
    if (Array.isArray(res)) {
      ...
      if (res.length > 0 && !boundModelId.value) {
...
const changeModel = (modelId) => {
  ...
  boundModelId.value = id

  // Si une conversation est déjà en cours, persister le(s) modèle(s) côté backend.
  // Sinon, on ne fait que mettre à jour la sélection pour la prochaine création.
  if (conversationId.value) {
    // best-effort: ne pas bloquer l’UI si l’API échoue
    chat.update(conversationId.value, {
      model_id: model_id.value || null,
      reasoning_model_id: reasoning_model_id.value || null,
    }).catch((e) => console.warn('[ModelSelect] update conversation model failed:', e))
  }
}
...
const selectedModel = computed(() => {
  ...
  const currentModelId = selectedModelValue.value ?? boundModelId.value ?? modelList.value[0]?.id ?? null
...
watchEffect(() => {
  ...
  if (selected && selected !== boundModelId.value && modelList.value && modelList.value.length > 0) {
