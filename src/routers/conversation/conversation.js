...
router.post("/", async ({ state, request, response }) => {
  ...
  const { content, mode_type, agent_id, model_id, reasoning_model_id } = body
...
router.put("/:id", async ({ state, params, request, response }) => {
  ...
  const body = request.body || {};
  const { title, model_id, reasoning_model_id } = body;
  ...
  try {
    ...
    // 1) Mise à jour du titre (compat historique)
    // - Si "title" est fourni (même vide), on conserve la logique auto-génération.
    if (title !== undefined) {
      let nextTitle = title;

      if (!nextTitle || nextTitle === "") {
        nextTitle = await auto_generate_title(
          conversation.dataValues.content,
          conversation.dataValues.conversation_id
        );
        if (nextTitle === 'ERR_BAD_REQUEST') {
    ...
    // 2) Mise à jour des modèles (nouveau)
    if (model_id !== undefined) {
      conversation.model_id = model_id;
    }
    if (reasoning_model_id !== undefined) {
      conversation.reasoning_model_id = reasoning_model_id;
    }
