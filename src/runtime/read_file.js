...
async function read_file(filepath) {
  try {
    ...
    // Preserve structured error info for upstream classification
    try {
      err.code = error && error.code ? error.code : undefined;
      err.errno = error && error.errno ? error.errno : undefined;
      err.syscall = error && error.syscall ? error.syscall : undefined;
      err.path = error && error.path ? error.path : filepath;
    } catch (e) {
      // ignore
    }
    throw err;
