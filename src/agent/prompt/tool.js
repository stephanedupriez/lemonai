...
const resolveToolPrompt = async () => {
  ...
  for (const [toolName, tool] of Object.entries(tools)) {
    if (toolName === 'web_search' || toolName === 'browser' || toolName === 'read_url') {
      continue;
    }
...
${toolDescription}
<tool finish>
{ "description": "Signal that a task goal is complete and explicitly declare whether it ended in SUCCESS or FAILED.", "name": "finish", "params": { "type": "object", "properties": { "status": { "description": "Mandatory goal outcome. Use SUCCESS when the goal is achieved, FAILED when it is not achieved or impossible under constraints.", "type": "string", "enum": ["SUCCESS", "FAILED"] }, "message": { "description": "Explanation of the task completion result", "type": "string" } }, "required": ["status", "message"] } }
</tool>
</tool_list>
...
- ${Object.keys(tools)
    .filter(name => !['web_search', 'browser', 'read_url'].includes(name))
    .join('\n  - ')}
- finish
</tool_call_guidelines>
